...
2. **Create the tables.** Left sidebar → **SQL Editor** → **New query**. Paste the
   ...
   **Run**. This creates `profiles`, `progress`, and `subscribers`, and turns on
   row-level security. You should see "Success. No rows returned." (Safe to
   re-run — if you set this up before the newsletter existed, re-running is how
   you get the `subscribers` table.)
