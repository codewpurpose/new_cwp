export function UseCasesSection() {
  return (
    <div id="about" className="scroll-mt-24">
      <section>
        <div className="mx-auto w-full max-w-[85rem] px-5 md:px-10">
          <h2 className="mb-8 text-[1.625rem] leading-[1.05] tracking-[-0.029em] md:mb-10 md:text-[2.5rem] md:leading-[0.98]">
            <span>Our Mission to Bring Knowledge Where It&apos;s Needed Most</span>
            <br />
            <span className="text-[#818181]">
              We are a community of students united by one belief — that quality
              coding education should cost nothing.
            </span>
          </h2>
          <div className="flex flex-col gap-6 md:gap-8">
            <article className="home-card grid overflow-hidden rounded-[20px] md:grid-cols-2">
              <div className="order-2 hidden min-w-0 p-6 pt-0 min-[375px]:block md:p-10 md:order-none">
                <div className="flex flex-col overflow-hidden rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f2f2f2] md:aspect-[782/521]">
                  <div className="flex h-7 shrink-0 items-center gap-1.5 border-b-[0.5px] border-[#e1e1e1] bg-white px-3">
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                  </div>
                  <div className="flex min-h-0 flex-1 flex-col px-4 pt-4 md:px-6 md:pt-6">
                    <div className="flex flex-1 gap-3 md:gap-4">
                      <div className="flex min-w-0 flex-1 flex-col gap-2 rounded-lg border-[0.5px] border-[#e1e1e1] bg-white p-3 md:p-4">
                        <div className="text-[8px] uppercase tracking-[0.12em] text-[#818181] md:text-[10px]">Support</div>
                        <div className="text-[10px] leading-snug text-[#1f1f1f] md:text-[12px]">What&apos;s our refund policy on annual plans?</div>
                        <div className="mt-auto rounded-md bg-[#dbefdb] p-2 text-[10px] leading-snug text-[#1f1f1f] md:p-2.5 md:text-[12px]">45-day full refund, pro-rated after.</div>
                      </div>
                      <div className="flex min-w-0 flex-1 flex-col gap-2 rounded-lg border-[0.5px] border-[#e1e1e1] bg-white p-3 md:p-4">
                        <div className="text-[8px] uppercase tracking-[0.12em] text-[#818181] md:text-[10px]">Sales</div>
                        <div className="text-[10px] leading-snug text-[#1f1f1f] md:text-[12px]">Can we refund a customer after 30 days?</div>
                        <div className="mt-auto rounded-md bg-[#dbefdb] p-2 text-[10px] leading-snug text-[#1f1f1f] md:p-2.5 md:text-[12px]">45-day full refund, pro-rated after.</div>
                      </div>
                    </div>
                    <div className="flex justify-center gap-16 md:gap-24">
                      <svg viewBox="0 0 2 20" preserveAspectRatio="none" aria-hidden="true" className="h-4 w-[2px] md:h-5">
                        <path className="home-flow-dash" d="M1 20V0" stroke="#3e7f5c" strokeWidth="1.5"></path>
                      </svg>
                      <svg viewBox="0 0 2 20" preserveAspectRatio="none" aria-hidden="true" className="h-4 w-[2px] md:h-5">
                        <path className="home-flow-dash" d="M1 20V0" stroke="#3e7f5c" strokeWidth="1.5"></path>
                      </svg>
                    </div>
                    <div className="mx-auto mb-4 flex items-center gap-2 rounded-lg border-[0.5px] border-[#cecece] bg-white px-3 py-2 md:mb-6">
                      <div className="h-3.5 w-3.5 shrink-0 rounded-[4px] bg-[#1e3c2c]"></div>
                      <div>
                        <div className="text-[9px] font-medium leading-tight text-[#1f1f1f] md:text-[11px]">Shared learning</div>
                        <div className="text-[8px] leading-tight text-[#818181] md:text-[10px]">Refund policy · decided May 12</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="order-1 flex min-w-0 flex-col justify-center p-6 md:order-none md:p-10 lg:p-12">
                <h3 className="max-w-[20ch] text-[1.375rem] leading-[1.1] md:text-[1.875rem]">Knowledge where it&apos;s needed most</h3>
                <p className="mt-4 max-w-[34rem] text-[15px] leading-[1.5] text-[#636363] md:text-base">
                  What began as a frustration with $15,000 bootcamps has grown into a
                  movement of free courses, real skills, and unwavering commitment.
                  We reach students in 110+ countries — from rural villages in India
                  to classrooms in Nigeria — and give them the same opportunities as
                  anyone else.
                </p>
                <div className="mt-6 rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f9f9f9] px-4 py-3.5 text-[14px] italic leading-[1.5] text-[#636363]">
                  110+ countries where students are already learning with us — for
                  free, with no strings attached.
                </div>
              </div>
            </article>
            <article className="home-card grid overflow-hidden rounded-[20px] md:grid-cols-2">
              <div className="order-2 hidden min-w-0 p-6 pt-0 min-[375px]:block md:p-10 md:order-2">
                <div className="flex flex-col overflow-hidden rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f2f2f2] md:aspect-[782/521]">
                  <div className="flex h-7 shrink-0 items-center gap-1.5 border-b-[0.5px] border-[#e1e1e1] bg-white px-3">
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                  </div>
                  <div className="flex min-h-0 flex-1 gap-2 p-3 md:gap-3 md:p-4">
                    <div className="flex w-[32%] min-w-0 flex-col justify-center gap-2 rounded-lg bg-[#ececec] p-2.5 md:p-3">
                      <div className="w-fit max-w-full rounded-full bg-[#1e3c2c] px-2 py-0.5 text-[8px] leading-tight text-[#dbefdb] md:text-[9px]">From CWP</div>
                      <div className="space-y-0.5 rounded-md border-[0.5px] border-[#e1e1e1] bg-white p-2">
                        <div className="text-[8px] font-medium leading-tight text-[#1f1f1f] md:text-[10px]">Decision</div>
                        <div className="text-[8px] leading-tight text-[#818181] md:text-[10px]">Writes use the service layer</div>
                      </div>
                      <div className="space-y-0.5 rounded-md border-[0.5px] border-[#e1e1e1] bg-white p-2">
                        <div className="text-[8px] font-medium leading-tight text-[#1f1f1f] md:text-[10px]">Lesson</div>
                        <div className="text-[8px] leading-tight text-[#818181] md:text-[10px]">Dedupe retried jobs</div>
                      </div>
                    </div>
                    <svg viewBox="0 0 20 2" preserveAspectRatio="none" aria-hidden="true" className="h-[2px] w-4 self-center md:w-6">
                      <path className="home-flow-dash" d="M0 1H20" stroke="#3e7f5c" strokeWidth="1.5"></path>
                    </svg>
                    <div className="flex min-w-0 flex-1 flex-col justify-center gap-2 rounded-lg border-[0.5px] border-[#e1e1e1] bg-white p-3 md:p-4">
                      <div className="flex items-center justify-between gap-2">
                        <div className="truncate text-[9px] font-medium text-[#1f1f1f] md:text-[12px]">Maya&apos;s first PR</div>
                        <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#dbefdb]">
                          <svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
                            <path d="M2 5.2 4.2 7.4 8 3" stroke="#1e3c2c" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"></path>
                          </svg>
                        </div>
                      </div>
                      <div className="mt-1 space-y-1.5 text-[9px] leading-snug md:text-[11px]">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[#1e3c2c]">+</span>
                          <span className="rounded-sm bg-[#dbefdb] px-1 text-[#1f1f1f]">uses the service layer</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[#1e3c2c]">+</span>
                          <span className="rounded-sm bg-[#dbefdb] px-1 text-[#1f1f1f]">dedupes retried jobs</span>
                        </div>
                        <div className="text-[#818181]">Opened day one · approved first pass</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="order-1 flex min-w-0 flex-col justify-center p-6 md:order-none md:p-10 lg:p-12">
                <h3 className="max-w-[20ch] text-[1.375rem] leading-[1.1] md:text-[1.875rem]">Volunteers who lean in to help</h3>
                <p className="mt-4 max-w-[34rem] text-[15px] leading-[1.5] text-[#636363] md:text-base">
                  Our volunteers don&apos;t just teach — they mentor, troubleshoot,
                  and celebrate every breakthrough. Hands-on learning in classrooms,
                  workshops, and one-on-one sessions makes coding feel approachable
                  for every student.
                </p>
                <div className="mt-6 rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f9f9f9] px-4 py-3.5 text-[14px] italic leading-[1.5] text-[#636363]">
                  Volunteer leaning in to help a young student at their laptop —
                  the kind of moment that defines CodeWithPurpose.
                </div>
              </div>
            </article>
            <article className="home-card grid overflow-hidden rounded-[20px] md:grid-cols-2">
              <div className="order-2 hidden min-w-0 p-6 pt-0 min-[375px]:block md:p-10 md:order-none">
                <div className="flex flex-col overflow-hidden rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f2f2f2] md:aspect-[782/521]">
                  <div className="flex h-7 shrink-0 items-center gap-1.5 border-b-[0.5px] border-[#e1e1e1] bg-white px-3">
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                    <span className="h-2 w-2 rounded-full bg-[#e1e1e1]"></span>
                  </div>
                  <div className="flex min-h-0 flex-1 items-center justify-center gap-2.5 px-4 py-6 md:gap-4 md:px-6 md:py-0">
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="h-9 w-9 rounded-full border-[0.5px] border-dashed border-[#818181] bg-[#e1e1e1] opacity-40 md:h-11 md:w-11"></div>
                      <div className="text-center text-[8px] leading-tight text-[#818181] md:text-[10px]">
                        Sam, staff eng
                        <br />
                        leaves Friday
                      </div>
                    </div>
                    <svg viewBox="0 0 20 2" preserveAspectRatio="none" aria-hidden="true" className="h-[2px] w-6 md:w-10">
                      <path className="home-flow-dash" d="M0 1H20" stroke="#3e7f5c" strokeWidth="1.5"></path>
                    </svg>
                    <div className="w-2/5 min-w-0 space-y-1.5 rounded-lg border-[0.5px] border-[#cecece] bg-white p-3 md:p-4">
                      <div className="inline-flex items-center rounded-full bg-[#1e3c2c] px-2 py-0.5 text-[8px] leading-none text-[#dbefdb] md:text-[9px]">Shared learning</div>
                      <div className="text-[9px] leading-snug text-[#1f1f1f] md:text-[11px]">Why we migrated to v2</div>
                      <div className="w-fit rounded-sm bg-[#dbefdb] px-1 text-[9px] leading-snug text-[#1f1f1f] md:text-[11px]">How to roll it back</div>
                    </div>
                    <svg viewBox="0 0 20 2" preserveAspectRatio="none" aria-hidden="true" className="h-[2px] w-6 md:w-10">
                      <path className="home-flow-dash" d="M0 1H20" stroke="#3e7f5c" strokeWidth="1.5"></path>
                    </svg>
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="h-9 w-9 rounded-full border-[0.5px] border-[#1e3c2c] bg-[#dbefdb] md:h-11 md:w-11"></div>
                      <div className="text-center text-[8px] leading-tight text-[#636363] md:text-[10px]">
                        Their replacement
                        <br />
                        ready Monday
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="order-1 flex min-w-0 flex-col justify-center p-6 md:order-none md:p-10 lg:p-12">
                <h3 className="max-w-[20ch] text-[1.375rem] leading-[1.1] md:text-[1.875rem]">Recognized for leadership and service</h3>
                <p className="mt-4 max-w-[34rem] text-[15px] leading-[1.5] text-[#636363] md:text-base">
                  Recognized by the U.S. House of Representatives, we work every
                  day to prove that student-led education can reach the highest
                  standards of impact and community service.
                </p>
                <div className="mt-6 rounded-xl border-[0.5px] border-[#e1e1e1] bg-[#f9f9f9] px-4 py-3.5 text-[14px] italic leading-[1.5] text-[#636363]">
                  &ldquo;Tremendous leadership and service to your community.&rdquo; —
                  Representative Mark DeSaulnier, U.S. House of Representatives
                </div>
              </div>
            </article>
          </div>
        </div>
      </section>
    </div>
  );
}
